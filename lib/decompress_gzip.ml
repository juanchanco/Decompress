...
struct
  ...
  module Crc32 = Decompress_crc32.Make(Char)(struct type elt = char include I end)
  ...
  and header deflater =
    ...
    let rec write_header0 header deflater =
      ...
      then begin
    put_byte deflater 0x1f;
    put_byte deflater 0x8b;
    put_byte deflater 8;
    put_byte deflater 0;
    put_int deflater  0;
    put_byte deflater 0;
    ...
    let rec aux deflater =
      let new_mode, read = match deflater.mode with
        | Flat (buffer, real_size, window_bits) ->
          ...
          (* update CRC *)
          deflater.crc <- Crc32.update deflater.src deflater.inpos len deflater.crc;
  ...
  and write_crc1 deflater =
    ...
    if deflater.needed > 1
    then begin
      let crc,_ = Crc32.get deflater.crc in

      put_int deflater crc;
  ...
  and write_crc2 deflater =
    ...
    if deflater.needed > 1
    then begin
      let _,length = Crc32.get deflater.crc in

      put_int deflater length;
      deflater.k <- ok;
